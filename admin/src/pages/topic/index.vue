<template>
  <div class="page-container">
    <div class="page-header">
      <h2>专题管理</h2>
      <el-button type="primary" :icon="Plus" @click="goEdit()">新增专题</el-button>
    </div>
    ...
    <el-table :data="list" v-loading="loading" border stripe>
      ...
      <el-table-column label="操作" width="150" fixed="right">
        <template #default="{ row }">
          <el-button link type="primary" @click="goEdit(row as Topic)">编辑</el-button>
          <el-button link type="danger" @click="handleDelete(row as Topic)">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
...
import { deleteTopic, listTopics, type TopicQuery } from '@/api/modules/topic'
...
const total = ref(0)
const query = ref<TopicQuery>({ keyword: '', status: '' })
...
async function loadViewCounts() {
  ...
  try {
    const counts = await getViewCounts('topic', ids)
...
function goEdit(row?: Topic) {
  router.push({ path: '/topic/edit', query: row ? { id: uuidToStr(row.id) } : {} })
}

async function handleDelete(row: Topic) {
  await ElMessageBox.confirm(`确定删除专题「${row.title}」吗？`, '删除确认', { type: 'warning' })
  await deleteTopic(uuidToStr(row.id))
