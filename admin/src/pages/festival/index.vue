<template>
  <div class="page-container">
    <div class="page-header">
      <h2>节日管理</h2>
      <el-button type="primary" :icon="Plus" @click="goEdit()">新增节日</el-button>
    </div>
    ...
    <el-table :data="list" v-loading="loading" border stripe>
      ...
      <el-table-column label="类型" width="100">
        <template #default="{ row }">{{ FESTIVAL_TYPE_MAP[row.type] || '-' }}</template>
      </el-table-column>
      <el-table-column prop="solarDate" label="公历" width="110" />
      <el-table-column prop="lunarDate" label="农历" width="110" />
      ...
      <el-table-column label="操作" width="150" fixed="right">
        <template #default="{ row }">
          <el-button link type="primary" @click="goEdit(row as Festival)">编辑</el-button>
          <el-button link type="danger" @click="handleDelete(row as Festival)">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
...
import { deleteFestival, listFestivals, type FestivalQuery } from '@/api/modules/festival'
import { getViewCounts } from '@/api/modules/stats'
import type { Festival } from '@/api/types'
import { CONTENT_STATUS_MAP, CONTENT_STATUS_OPTIONS, FESTIVAL_TYPE_MAP } from '@/constants'
...
const list = ref<Festival[]>([])
const total = ref(0)
const query = ref<FestivalQuery>({ keyword: '', status: '' })
...
async function loadViewCounts() {
  ...
  try {
    const counts = await getViewCounts('festival', ids)
...
function goEdit(row?: Festival) {
  router.push({ path: '/festival/edit', query: row ? { id: uuidToStr(row.id) } : {} })
}

async function handleDelete(row: Festival) {
  await ElMessageBox.confirm(`确定删除节日「${row.name}」吗？`, '删除确认', { type: 'warning' })
  await deleteFestival(uuidToStr(row.id))
